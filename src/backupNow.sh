# ----------------------------------------------------- #
# Backup now: Backs up the changed tracked files
# ----------------------------------------------------- #
...
#get loading bar increment
let loadingBarInc=$(wc -l "$dbFileLocation/database.db" | cut -d ' ' -f 1)
#iterate through the database and backup files
COUNT=`expr 100 / $loadingBarInc`
(
while read lineDB
do
  #get filename
